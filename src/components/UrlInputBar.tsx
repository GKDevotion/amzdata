...
import { Search, RotateCcw, ArrowRight, AlertCircle, Link2, X } from 'lucide-react';
...
export const UrlInputBar: React.FC<UrlInputBarProps> = ({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    if (!url.trim()) {
      setError('Please enter an Amazon product URL or 10-character ASIN.');
      return;
    }
    setError(null);
    onScrape(url.trim());
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
        <div>
          <h2 className="text-sm sm:text-base font-bold text-stone-900 flex items-center gap-2">
            ...
            <span>Amazon Product URL or ASIN Input</span>
          </h2>
        ...
        <div className="flex flex-col sm:flex-row gap-2">
          <div className="relative flex-1 flex items-center">
            ...
            <input
              ...
              placeholder="e.g. https://www.amazon.in/dp/B0GGHFSWCP or enter ASIN directly..."
              disabled={isLoading}
          ...
          <div className="mt-3 p-3 bg-amber-50/90 border border-amber-200 rounded-xl flex items-center justify-between text-xs text-amber-900">
            <div className="flex items-center gap-2">
              ...
              <span className="font-medium">
                {loadingStage || 'Extracting Amazon Product Details, FHD Images, and Specifications...'}
              </span>
            </div>
            <span className="text-amber-700 font-mono text-[11px] hidden sm:inline">Connecting to Amazon Live DOM</span>
          </div>
        )}
      </form>
    </section>
